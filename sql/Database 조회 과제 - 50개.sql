...
SELECT CITY
  FROM LOCATIONS 
 WHERE LOCATION_ID NOT IN (SELECT DISTINCT D.LOCATION_ID  
 					  		 FROM EMPLOYEES E
 							INNER JOIN DEPARTMENTS D
 					 		   ON E.DEPARTMENT_ID = D.DEPARTMENT_ID)
;
...
SELECT CITY
  FROM LOCATIONS 
 WHERE LOCATION_ID IN (SELECT DISTINCT D.LOCATION_ID  
 					  	 FROM EMPLOYEES E
 						INNER JOIN DEPARTMENTS D
 					 	   ON E.DEPARTMENT_ID = D.DEPARTMENT_ID)
;
...
SELECT D.DEPARTMENT_NAME AS D_NAME
	 , AVG(E.SALARY)
  FROM EMPLOYEES E
 INNER JOIN DEPARTMENTS D
	ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
...
SELECT *
 ...
 WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
 						   FROM (SELECT DEPARTMENT_ID 
 						  			  , RANK() OVER(ORDER BY LENGTH(DEPARTMENT_NAME)DESC) AS RNK
  								   FROM DEPARTMENTS)
 						  WHERE RNK = 1)
;
...
SELECT EMPLOYEE_ID 
	 , SALARY 
	 , SALARY * 0.1 * (TO_CHAR(SYSDATE, 'YYYY') - TO_CHAR(HIRE_DATE, 'YYYY'))
  FROM EMPLOYEES
;
...
SELECT J.JOB_TITLE AS "현재 직무명"
	 , NVL(JH_J.JOB_TITLE,'없음') AS "과거 직무명"
  FROM EMPLOYEES E
 INNER JOIN JOBS J
    ON E.JOB_ID = J.JOB_ID
  LEFT OUTER JOIN (SELECT JH.EMPLOYEE_ID 
  					 ...
  					 FROM JOB_HISTORY JH
  					INNER JOIN JOBS J2
  					   ON JH.JOB_ID = J2.JOB_ID) JH_J
    ON E.EMPLOYEE_ID = JH_J.EMPLOYEE_ID
;
-- 43. 직무가 변경될 때마다 급여가 15%씩 감소한다고 했을 때, 직무가 변경된 사원들의 감소된 급여를 조회한다.
SELECT E.SALARY * POWER(0.85,CNT)
  FROM EMPLOYEES E
 INNER JOIN (SELECT EMPLOYEE_ID 
				  , COUNT(EMPLOYEE_ID) AS CNT
			   FROM JOB_HISTORY 
			  GROUP BY EMPLOYEE_ID) JH_CNT
	ON E.EMPLOYEE_ID = JH_CNT.EMPLOYEE_ID 
;
...
SELECT TO_CHAR(HIRE_DATE,'YYYY') AS HD_YEAR
	 , COUNT(EMPLOYEE_ID)
  FROM EMPLOYEES
